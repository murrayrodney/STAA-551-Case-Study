...
Let's take a look at these singularities, I will see how many observations we have for each 
unique group for education and ethnicity
```{r}
data %>% 
  group_by(education, ethnicity) %>% 
  count()
```
From the above we can see there are some combinations between education and ethnicity that don't exist, after
seeing this it is no surprise that we are seeing NA's from singularities.
