...
n_vars <- 1:length(numerical_cols)
for (i in n_vars) {
  formulas <- bind_rows(formulas, get_formulas(numerical_cols, n_var=i))
  formulas <- bind_rows(formulas, get_formulas(numerical_cols, n_var=i, interactions=2))
}
...
for (i in 1:nrow(formulas)) {
  ...
  if (i == 1) {
    numerical_perf <- track_model_perf(model, as.character(i))
  } else {
    numerical_perf <- track_model_perf(model, as.character(i), numerical_perf)
  }
}
numerical_perf$interaction <- factor(formulas$interaction)
