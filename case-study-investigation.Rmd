...
track_model_perf <- function(model, new_model_name, prev_model_perf=NULL) {
  ...
  model_perf <- data.frame(
    ...
    r_squared_mult=sum_obj$r.squared,
    r_squared_adj=sum_obj$adj.r.squared,
    f_calc=sum_obj$fstatistic['value'],
    aic=AIC(model),
    bic=BIC(model)
    )
  
  if (!is.null(prev_model_perf)) {
    # prev_model_perf <- filter(prev_model_perf, formula != new_formula)
...
select(plot_model_perf, model_name, r_squared_adj, aic)
...
fig <- ggplot(plot_model_perf, aes(x=r_squared_adj, y=aic, label=model_name)) +
