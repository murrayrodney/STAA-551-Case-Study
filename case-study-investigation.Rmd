...
```
```{r}
summary(select(data, smoking, chol))
```
...
forward_aic <- step(base_model, scope=scope, direction='forward', data=data)
```
```{r}
summary(forward_aic)
plot(forward_aic)
...
backward_aic <- step(full_model, scope=scope, direction='backward', data=data)
...
forward_aic_poly <- step(base_model, scope=scope, direction='forward', data=data)
```
...
# backward_aic_poly <- step(poly_model, scope=scope, direction='backward', data=data, trace=-1 )
```
